...
- **Sticky session + UK by default:** For Decodo residential, a **sticky** session keeps the **same egress IP** for N minutes (default **60**, minimum **30** for auto-provision; see [Decodo sticky sessions](https://help.decodo.com/docs/residential-proxy-custom-sticky-sessions)). The gate username gets `-country-gb` **by default** (UK exit), then `-session-{stableId}-sessionduration-{minutes}` — **one stable session id per (client, IG handle)**. Override country with `DECODO_GATE_COUNTRY` (e.g. `us`) or disable pinning with `DECODO_GATE_COUNTRY=none`. The gate username uses Decodo’s **`user-` prefix** by default. **Reconnect** after changing env so `proxy_url` updates.
